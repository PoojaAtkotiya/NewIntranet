...
export default class SimplePoll extends React.Component<ISimplePollProps, ISimplePollState> {
    ...
    constructor(props: ISimplePollProps, context: IWebPartContext) {
        super(props, context);
        //Save the context
    ...
    private closeVote(): void {
        this.setState({ popupOpened: false, alreadyVote: true, hideDialog: true })
      }
    ...
    private loadChart(): void {
        ...
        this.setState({ resultsLoaded: true, viewResults: true })
     }

    private loadQuestions(props: ISimplePollProps): void {
         ...
         //Request the existing votes to get current user voting status
            listService.getVoteForUser(props.surveyList, item.StaticName, this.myPageContext.pageContext.user.loginName).then((responseVote) => {
                ...
                if (responseVoteVal.length > 0) {
                    this.setState({ alreadyVote: true, selectedValue: responseVoteVal[0].Title });
                }
                else {
     ...
     */
    public componentWillReceiveProps(nextProps: ISimplePollProps): void {
