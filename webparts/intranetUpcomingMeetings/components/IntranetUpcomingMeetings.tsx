...
import styles from './IntranetUpcomingMeetings.module.scss';
import { escape } from '@microsoft/sp-lodash-subset';
import * as MicrosoftGraph from '@microsoft/microsoft-graph-types';
import * as strings from 'IntranetUpcomingMeetingsWebPartStrings';
import { IIntranetUpcomingMeetingsProps, IIntranetUpcomingMeetingsState, IMeeting, IMeetings } from '.';
//Stylings
...
export default class IntranetUpcomingMeetings extends React.Component<IIntranetUpcomingMeetingsProps, IIntranetUpcomingMeetingsState> {
 private _interval: number;

  constructor(props: IIntranetUpcomingMeetingsProps) {
    super(props);
  ...
  */
  ...
  private _loadMeetings(): void {
    ...
    date.setDate(date.getDate() + (this.props.daysInAdvance || 0));//(10 || 0));//
...
*/
  ...
  public componentDidUpdate(prevProps: IIntranetUpcomingMeetingsProps, prevState: IIntranetUpcomingMeetingsState): void {
  ...
  public render(): React.ReactElement<IIntranetUpcomingMeetingsProps> {
    return (
      <div className={styles.intranetUpcomingMeetings}>
        <div className={styles.container}>
        <img src={`${logo}`} className={styles.rightalig} />
          <p className={styles.header}>Upcoming Meetings</p>
            ...
            this.state.meetings &&
              this.state.meetings.length > 0 ? (
                <div className={styles.padding}>
                  ...
                  <Link href='https://outlook.office.com/owa/?path=/calendar/view/Day' target='_blank' className={styles.viewAll}>
                    <img src={`${arrow}`} className={styles.viewAll} /></Link>
                </div>
              ) : (
                !this.state.loading && (
                  this.state.error ?
